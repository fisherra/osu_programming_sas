...
data fire;
input station_num response_time @@;
datalines;
1 15 1 17 1 22 1 19 1 25
2 13 2 9 2 14 2 11 2 15
3 17 3 21 3 15 3 23 3 18
4 10 4 8 4 11 4 11 4 12
;


proc npar1way data=fire wilcoxon plots=none;
	class station_num;
	var response_time;
	title3 "Question 1a";
run;

proc glm data=fire plots(only)=boxplot;
	class station_num;
	model response_time = station_num;
	means station_num / plots=none;
	title3 "Question 1b";
run;
...
proc rank data = mesonet18 ties = high out = mesonet18_rank_1;
	var MAXHUMID;
	ranks max_hum_rank;
	title3 "Question 2a Maximum Humidity Ranks";
proc print data = mesonet18_rank_1 noobs;
	var Month day MAXHUMID max_hum_rank;
	where max_hum_rank < 11;
run;
...
proc rank data = mesonet18 ties = high out = mesonet18_rank_1;
	var MINTEMP;
	ranks min_temp_rank;
	title3 "Question 2a Minimum Temperature Ranks";
proc print data = mesonet18_rank_1 noobs;
	var Month day MINTEMP min_temp_rank;
	where min_temp_rank < 11 and MINTEMP > 0;
run;
