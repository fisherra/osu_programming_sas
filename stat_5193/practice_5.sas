...
proc rank data = nba_full descending ties = low out= nba_ranked;
	by year; 
	var Points;
	ranks rank_by_points;
run;

proc print data=nba_ranked;
	var team Points rank_by_points;
	Title3 'Question 1b';
run; 
...
proc npar1way data = soy wilcoxon plots = wilcoxonboxplot;
	class season;
	var yield;
	title "Question 2a";
run;
...
Proc glm data = soy plots = residuals;
	class season;
	model yield = season;
	title "Question 2b";
run;

/* My residual plot isn't showing up, so I'm not sure if it's normal.. */
