...
proc means data = mesonet18;
var MAXTEMP MINTEMP MAXHUMID MINHUMID SPEEDAVG; 
output out = mesonet18_means; /* need to secribe the fiv vars?*/
title "Fisher Ankney";
title2 "Assignment 3";
title3 "Question 1";
proc print data = mesonet18_means;
...
by Month;
title3 "Question 2";
proc means data = mesonet18 mean range cv std;
var MAXHUMID;
by Month;
output out = max_humidity_stats mean=mean_maxhumidity cv = cv_maxhumidity;
proc print data = max_humidity_stats;





proc means data = mesonet18 min max mean;
var SPEEDAVG;
class Month;
output out = windspeed_avg min = windspeed_avg_min max = windspeed_avg_max;
title3 "Question 3";
proc print data = windspeed_avg;


/* explain how class month differs from by month here */



data Prob4; 
set mesonet18;
mean_temp_function = mean(MINTEMP, MAXTEMP);
mean_temp_arithmatic = (MINTEMP + MAXTEMP) / 2;
...
/* 4b */
/* difference is the function will ignore NA's and just give you the single temperature you have, while the arithmatic NA's transfer and give you an NA outcome */


/* 4c */
/* proc means or proc univariate will calculate the means per each variable, not the combined mean of the variables. */
