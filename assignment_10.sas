...
proc print data = CLASS.MESONET1_4 label;
	title 'Question 4';
	where Month = "Mar";
run;
...
proc means data = CLASS.MESONET1_4 mean;
	class Month;
	var MAXTEMP MINTEMP MAXHUMID MINHUMID SPEEDAVG RAIN_IN STNPRESSUR SOLAR_MJ_m2 SODSOIL BARESOIL;
	output out = WORK.sumstats
	mean = m_max_t m_min_t m_max_h m_min_h m_speed m_rain m_pressure m_solar m_sod_soil m_bare_soil; 
	title 'Question 5a';
run;
...
PROC EXPORT DATA= WORK.SUMSTATS 
            OUTFILE= "C:\Users\fankney\Downloads\Assign10 Data.xlsx" 
