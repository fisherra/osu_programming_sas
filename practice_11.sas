...
PROC EXPORT DATA= CLASS.MESONET1_4 
            OUTFILE= "C:\Users\fankney\Downloads\Assignment_11_data.xlsx" 
            DBMS=EXCEL REPLACE;
     SHEET="a11"; 
RUN;
...
data class.meso_c;
	set class.Mesonet1_4;
		/* create an array a{number of var} for input names */
		array a{4} SODSOIL BARESOIL MAXTEMP MINTEMP;
		/* create an array b{number of var} for output names */
		array b{4} sodsoilc baresoilc maxtempc mintempc;
		/* do from first variable number to last variable number */
		do i=1 to 4;
		/* define the function to do to the input array */
		b{i}=(5/9)*(a{i} - 32);
end;


label
	sodsoilc='Avg Temp Sodsoil in C'
	baresoilc='Avg Temp Bare Soil in C'
	maxtempc='Max Temp in C'
	mintempc='Min Temp in C';
run;

proc print data=class.meso_c label;
run;
...
proc gchart data = ran1;
	vbar x / type = mean levels = 10;
run;
...
proc gchart data = ran2;
	vbar y / type = mean levels = 10;
run;
...
proc gchart data = ran3;
	vbar z / type = mean levels = 10;
run;
