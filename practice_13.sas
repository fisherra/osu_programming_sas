...
proc gplot data = class.mesonet1_4;
	plot maxtemp * sodsoil;
	symbol1 value = square cv = brown I = none;
RUN;
...
proc gplot data = class.mesonet1_4;
	where month = 'Jan' or month = 'Feb' or month = 'Mar' or month = 'Apr';
	plot maxtemp * sodsoil = month;
	symbol1 value = square cv = blue I = none;
	symbol2 value = circle cv = red I = none;
	symbol3 value = triangle cv = green I = none;
	symbol4 value = diamond cv = purple I = none;
	title3 'Question 1b';
run;
...
proc sort data = class.mesonet1_4;
	by Month;
run;
...
proc gplot data = class.mesonet1_4;
	where month = 'Jan' or month = 'Feb';
	by month;
	PLOT (maxtemp sodsoil) * day / OVERLAY ;
	symbol1 interpol=join value = none;
	symbol2 interpol=join value = none;
	title3 'Question 2';
...
proc gchart data = class.mesonet1_4;
	hbar month / sumvar = rain_in;
	pattern1 color = blue;	
	title3 'Question 3';
run;
...
/* histogram of two soil temp where bars are grouped by 
month for each ereponse */

proc gchart data = class.mesonet1_4;
	where month = 'Jan' or month = 'Feb' or month = 'Mar' or month = 'Apr';
	vbar sodsoil / group = month;
	pattern1 color = green;
	title3 'Question 4 sodsoil';
run;

proc gchart data = class.mesonet1_4;
	where month = 'Jan' or month = 'Feb' or month = 'Mar' or month = 'Apr';
	vbar baresoil / group = month;
	pattern1 color = purple;
	title3 'Question 4 baresoil';
run;
